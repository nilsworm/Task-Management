...
async def test_save_and_get_sprint_task(repo: PostgresTaskRepository, factory: TaskFactory) -> None:
    sid = uuid.uuid4()
    task = factory.create_sprint("API endpoint", sprint_id=sid, estimation=Estimation(5))
    await repo.save(task)
...
async def test_save_and_get_milestone(repo: PostgresTaskRepository, factory: TaskFactory) -> None:
    gid = uuid.uuid4()
    task = factory.create_milestone("v1.0", due_date=date(2026, 3, 31), goal_id=gid)
    await repo.save(task)
...
async def test_list_by_sprint(repo: PostgresTaskRepository, factory: TaskFactory) -> None:
    sid = uuid.uuid4()
    t1 = factory.create_sprint("In sprint", sprint_id=sid)
    t2 = factory.create_sprint("Other sprint", sprint_id=uuid.uuid4())
    ...
    results = await repo.list_by_sprint(sid)
